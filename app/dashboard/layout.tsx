...
const NAV = [
  ...
  { href: "/dashboard/tts",       icon: "🔊", label: "Text to Speech" },
];
...
function Sidebar({ credits, userEmail }: { credits: number | null; userEmail: string }) {
  ...
  return (
    <aside className="fixed top-0 left-0 h-screen w-16 md:w-56 bg-[#0e0e0e] border-r border-white/5 flex flex-col z-40 transition-all">
      {/* Logo */}
      <div className="h-14 flex items-center justify-center md:justify-start px-3 md:px-4 border-b border-white/5 flex-shrink-0">
        ...
        <span className="hidden md:block ml-2.5 text-white font-bold text-sm tracking-wide">InfiMagen</span>
      </div>
        ...
        {NAV.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex items-center gap-3 px-2 md:px-3 py-2.5 rounded-xl text-sm transition-all group relative ${
                active
      ...
      <div className="p-2 md:p-3 border-t border-white/5 space-y-2 flex-shrink-0">
        ...
        {/* User */}
        <div className="flex items-center gap-2.5 px-1 md:px-2 py-1 rounded-xl hover:bg-white/5 transition-colors cursor-pointer group" onClick={signOut} title="Sign out">
          ...
          <div className="hidden md:block flex-1 min-w-0">
            ...
            <p className="text-xs text-white/20 group-hover:text-red-400 transition-colors">Sign out</p>
          </div>
        </div>
      </div>
    </aside>
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#080808]">
      ...
      <main className="flex-1 ml-16 md:ml-56 min-h-screen overflow-y-auto">
        {children}
      </main>
    </div>
  );
